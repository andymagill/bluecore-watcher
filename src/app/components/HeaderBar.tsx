  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-800 pb-4">
      <div>
        ...
        <p className="text-sm text-neutral-500">{manifest.entities.map((e) => e.name).join(", ")}</p>
      </div>
      <div className="flex items-center gap-3">
        <span className="text-xs text-neutral-500">Last run {new Date(manifest.generatedAt).toLocaleString()}</span>
