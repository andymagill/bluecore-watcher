...
- `--destructive` is overridden in both themes. The export's light value (`oklch(0.9237 0.1811 109.4770)`) is a bright yellow-green that fails the 4.5:1 contrast bar (`04-FRONTEND.md` §7) and doesn't read as an error; its dark value is identical to `--secondary`. Replaced with an actual red in both themes.
...
- `--sidebar-*` was dropped (no sidebar in this app), as was the export's `--font-sans: Geist Mono` — see below.
...
**Fonts.** Geist Sans and JetBrains Mono are self-hosted via `@fontsource-variable/*` (imported in `main.tsx`), not linked from Google Fonts. The smoke gate (`src/gate/smoke-render.ts`) waits on `networkidle` and fails on any console error, so a third-party font request during CI or preview is an avoidable extra failure mode; this is also a diligence tool opened inside client networks with egress restrictions. `--font-sans` is set to Geist **Sans**, not Geist Mono as the export specifies — the export's choice makes the entire UI monospace (headings and markdown prose included), which favors the dense numeric look at the cost of prose readability. Geist/JetBrains Mono are still used for values, anchors, and raw provenance text via `--font-mono`.
