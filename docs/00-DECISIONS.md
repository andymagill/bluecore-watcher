...
- Full design and the remaining M3 workstreams (repair tooling, the failing-data safety valve resolving Q4's safety half, the fire-drill exit proof) are tracked in `docs/plans/m3-operability.md`. Q4's SLA/UX half (analyst-facing copy) is deferred past M3 — see `07-ROADMAP.md` Deferred.
...
| Q4  | What is the honest weekly maintenance budget, and what does the client see when a source has been broken for six weeks? **Split 2026-09-14:** the safety half (long-broken data must stop rendering as current) is in scope for M3c; the SLA/UX half (analyst-facing copy, an outage note, a client sees this at all) needs an actual analyst and is not yet resolved. See `docs/plans/m3-operability.md` M3c and `07-ROADMAP.md` Deferred. | Weekly maintenance budget: `06-OPS-RUNBOOK.md` §9. Safety half: M3c. SLA/UX half: deferred, needs M4 auth first.  | Andrew   |
