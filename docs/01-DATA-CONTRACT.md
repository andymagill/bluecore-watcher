...
**A composite `api` location's block (ADR-022, `02-CONFIG-SCHEMA.md` §3)** is otherwise an ordinary scalar block (`presenter`/`type` are always `"markdown"`), but `rawText` is a stable-key JSON snapshot of the _raw_, pre-transform field values rather than a single string, and `anchor` names the resolved `index` path plus every field's resolved path (`{index}` substituted). `value`/`displayValue` are the composed markdown; `contentHash` hashes `rawText`, same as any other block — so it's still true that a `valueMap` label or `template` wording change never moves `contentHash`, only `value`.
