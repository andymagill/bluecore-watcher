...
No source triaged here yet. `fr-doe-nuclear` previously sat in this section under its old "Market Climate" framing but was reclassified as regulatory signal (see the Regulatory & Policy table above). The intended fit for this section is EIA's regional price-index APIs (the shape the synthetic `example.config.ts` target mimics), which require a free `api_key` (confirmed: `api.eia.gov` 403s without one) — deferred to M2 rather than adding a secret for this pass. Until then this section renders a zero-state.
