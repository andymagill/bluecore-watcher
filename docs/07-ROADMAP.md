...
Add sources that expose regulatory and procurement content currently unavailable through the public web dashboard. In progress across two sequential workstreams (M5a delivered; M5b not started) — see [`docs/plans/m5-new-source-types.md`](plans/m5-new-source-types.md) for the plan, live-triage findings, and status.
...
**Status:** M5a delivered 2026-09-17 (`viewUrl`/rule 14 plus `usaspending-advanced-reactor-awards`, live-verified — `npm run fixture:verify` reports 11 targets ok, zero drift on the 10 pre-existing). M5b (maritime FR target, close-out) not started; see the plan doc for the split.
