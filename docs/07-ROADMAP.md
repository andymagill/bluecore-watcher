...
- **Company** (`bluecore-newsroom`): `latest_headline`/`latest_post_category` become a 3-item `recent_posts` list; `latest_post_date`'s monotonic guard stays. ✅ M6a.
...
**Status:** In progress. **M6a delivered 2026-09-17** (ADR-025 written in `00-DECISIONS.md`): the composite `list` location for `html`, `bluecore-newsroom` converted to a 3-item `recent_posts` list, and M4c (`bluecore-form-d` retirement) folded in. M6b (`kind: "xml"`, competitor press-release targets) is Next; ADR-026 lands when it does, per house convention.
